...
import com.example.studentmanagement.adapter.SpinnerAdapter;
import com.example.studentmanagement.adapter.TimlineAdapter;
import com.example.studentmanagement.model.ChooseExam;
import com.example.studentmanagement.model.OnlineExam;
import com.example.studentmanagement.model.Result;
import com.example.studentmanagement.model.TimeLine;
...
public class ExamChooseActivity extends AppCompatActivity {
    ...
    void getAllOnlineExamName(final int id){
...
//                        Toast.makeText(getApplicationContext(),title +" "+array.length(), Toast.LENGTH_SHORT).show();
                ...
                if(examNames.size() > 0){
                    ...
                    ArrayAdapter adapter1 = new ArrayAdapter(ExamChooseActivity.this,R.layout.spinner_row_layout,R.id.spn_text,examNames);
    ...
    void getExamResult(int id,int code){
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, MyConfig.getStudentOnlineResult(id,code), null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                ...
                try {
                    if(response.getBoolean("success")){
                        ...
                        for(int i = 0 ; i < array.length() ; i++){
                            ...
                            int pass_mark = array.getJSONObject(i).getInt("pass_mark");
...
//                        Toast.makeText(getApplicationContext(),title +" "+array.length(), Toast.LENGTH_SHORT).show();
                ...
                if(results.size() > 0){
                    ...
                    adapter = new ResultAdapter(results,ExamChooseActivity.this);
